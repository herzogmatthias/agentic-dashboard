...
export async function checkOpenAPICompleteness() {
  ...
  for (const [p, obj] of Object.entries<any>(doc.paths ?? {})) {
    for (const [method, op] of Object.entries<any>(obj)) {
      if (op?.operationId) found.add(op.operationId);
    }
  }

  const missing = [...expected].filter(x => x && !found.has(x));
  if (missing.length) throw new Error(`OpenAPI missing operationIds: ${missing.join(", ")}`);
}